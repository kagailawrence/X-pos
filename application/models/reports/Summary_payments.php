...
class Summary_payments extends Summary_report
{
	...
	public function getData(array $inputs)
	{
		...
		$this->db->from('pos_sales AS sales');
		...
		// consider M-pesaas only one type of payment and do not show "Gift Card: 1, Gift Card: 2, etc." in the total
		$gift_card_count = 0;
		$gift_card_amount = 0;
		...
		if ($gift_card_count > 0) {
			$payments[] = array(
				'trans_group' => $this->lang->line('reports_trans_payments'),
				'trans_type' => $this->lang->line('sales_giftcard'),
	...
	protected function create_summary_payments_temp_tables($where)
	{
		...
		$this->db->query(
			'CREATE TEMPORARY TABLE IF NOT EXISTS ' . $this->db->dbprefix('sumpay_taxes_temp') .
		...
		$this->db->query(
			'CREATE TEMPORARY TABLE IF NOT EXISTS ' . $this->db->dbprefix('sumpay_items_temp') .
			' (INDEX(sale_id)) ENGINE=MEMORY
			(
		...
		$this->db->query(
			'CREATE TEMPORARY TABLE IF NOT EXISTS ' . $this->db->dbprefix('sumpay_payments_temp') .
			' (INDEX(sale_id)) ENGINE=MEMORY
			(
