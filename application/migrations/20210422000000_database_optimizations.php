...
class Migration_database_optimizations extends CI_Migration
{
	...
	public function up()
	{
		...
		foreach ($attribute_values->result_array() as $attribute_value) {
			$attribute_links = $this->db->query('SELECT links.definition_id, links.item_id, links.attribute_id, defs.definition_type FROM pos_attribute_links links JOIN pos_attribute_definitions defs ON defs.definition_id = links.definition_id where attribute_id = ' . $attribute_value['attribute_id']);
