...
UPDATE `pos_app_config` SET `key` = 'receipt_show_total_discount' WHERE `key` = 'show_total_discount';

DELETE FROM `pos_app_config` WHERE `key` = 'use_invoice_template';
DELETE FROM `pos_app_config` WHERE `key` = 'language';
DELETE FROM `pos_app_config` WHERE `key` = 'thousands_separator';
...
UPDATE `pos_modules` SET `sort` = 110 WHERE `name_lang_key` = 'module_config';
...
UPDATE `pos_employees` SET `hash_version` = 1;
